...
class PaginatedResponse(object):
    ...
    def __init__(self, client, endpoint, data, dataclass_object: dataclass):
        self._client = client
        self._dataclass_object = dataclass_object
        self.data = [dataclass_object.from_dict(datalist['attributes']) for datalist in data['data']]
    ...
    def __next__(self):
        ...
        if self._next_page_exists(self.meta):
            ...
            response = self._client._api_request(endpoint=self.endpoint,
                                                 params=params)
            self.data = [self._dataclass_object.from_dict(datalist['attributes']) for datalist in response['data']]
